...
if __name__ == "__main__":
    # 从环境变量获取配置信息
    app_id = "cli_a8e4ff16f178100d"
    app_secret = "g9y5khS5xR1jAZuw48xDHfVbh436lMuT"
    ...
    try:
        ...
        for record in all_records:
            ...
            for key in ["主讲", "助教"]:
                ...
                # 支持字符串和列表类型
                if isinstance(value, str) and "刘智耿" in value:
                    found = True
                elif isinstance(value, list):
                    # 可能是 [{'name': ...}, ...] 或字符串列表
                    for item in value:
                        if isinstance(item, dict) and "name" in item and "刘智耿" in item["name"]:
                            found = True
                        elif isinstance(item, str) and "刘智耿" in item:
                            found = True
                if found:
                    break
            if found:
